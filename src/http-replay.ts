...
import { cassetteEra, readCassette, type Cassette, type Era } from "./cassette.js";
import { isRequest, parseFrame, type JsonRpcRequest, type JsonRpcResponse } from "./jsonrpc.js";
...
/**
 * Fingerprints whose recorded answer is a stream. The cassette *has* an answer
 * for these, so calling them a miss would be a lie; they get an error that says
 * what is actually true.
 */
function streamedFingerprints(cassette: Cassette): Set<string> {
  ...
  for (const entry of cassette.entries) {
    if (entry.type !== "chunks" || entry.id === undefined) continue;
    const answered = requests.get(String(entry.id));
    if (answered) streamed.add(fingerprint(answered));
  }
  return streamed;
}
...
export async function startHttpReplay(cassettePath: string, opts: HttpReplayOptions = {}): Promise<ReplayServer> {
  ...
  const streamed = streamedFingerprints(cassette);
  ...
  const allow = sessioned ? "POST, DELETE" : "POST";
  ...
  const handlePost = (req: http.IncomingMessage, res: http.ServerResponse, body: Buffer) => {
    ...
    if (streamed.has(fingerprint(index.redactRequests ? (redactFrame(frame) as JsonRpcRequest) : frame))) {
      send(res, 200, {
        jsonrpc: "2.0",
        id: frame.id,
        error: {
          code: -32603,
          message: `mcp-cassette replay: the recorded answer for "${frame.method}" is streamed; SSE replay lands in PR 7`,
        },
      });
      return;
    }
    misses++;
    const diagnosis = diagnoseMiss(index, frame);
  ...
  return new Promise<ReplayServer>((resolve, reject) => {
    ...
    server.listen(port, host, () => {
      ...
      if (streamCount > 0) warn(`${streamCount} streamed answer(s) in the cassette — served in PR 7`);
      resolve({
        url: bound,
        misses: () => misses,
